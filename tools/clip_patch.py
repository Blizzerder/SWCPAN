...
def parse_args():
    ...
    parser.add_argument('-n', required=True, type=int, help='total number of images pairs')         #总图像对数
    parser.add_argument('-p', '--patch_num', required=True, type=int,
                        help='random clip how many patches for one training scene')                 #一个图中的patch数
...
if __name__ == "__main__":
    ...
    if not args.no_low_train:
        # train low-res
        cnt = 0
        out_dir = f"{args.data_dir}/Dataset/{args.satellite}/train_res"
    ...
    if not args.no_low_test:
        # test low-res
        cnt = 0
        out_dir = f"{args.data_dir}/Dataset/{args.satellite}/test_res"
        mmcv.mkdir_or_exist(out_dir)
        record = open(f'{out_dir}/record.txt', "w")
