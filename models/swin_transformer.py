...
class WindowAttention3(nn.Module):
    ...
    def forward(self, x_hr, x_lr, x_c, mask=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        ...
        k = self.k(x_c).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        v = self.v(x_hr).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class SwinTransformerBlock3(nn.Module):
    ...
    def forward(self, x_lr, x_hr, x_c):
        H, W = self.input_resolution
        B, L, C = x_lr.shape
        assert L == H * W, "input feature has wrong size"
...
class BasicLayer3(nn.Module):
    ...
    def forward(self, x_lr, x_hr, x_c): #修改，删除x_diff
        #x_lr = self.norm1(x_lr)
        x_hr = self.norm2(x_hr)
        x_c = self.norm3(x_c)

        for blk in self.blocks:
            x_lr = blk(x_lr, x_hr, x_c)
        return x_lr
