...
class FineUp(nn.Module):
    ...
    def forward(self, diff0_features, fine_features, coarse_features):
        #diff0_features, diff1_features, fine_features, coarse_features -> diff_features, fine_features,coarse_features 
        x0 = fine_features[4]   #保留
        x0 = self.layer(diff0_features[4], x0, coarse_features[4]) + x0  #修改，删除diff1
        x1 = self.up1(fine_features[3], diff0_features[3], coarse_features[3], x0)   #修改，删除diff1
        x2 = self.up2(fine_features[2], diff0_features[2], coarse_features[2], x1)   #修改，删除diff1
        x3 = self.up3(fine_features[1], diff0_features[1], coarse_features[1], x2)   #修改，删除diff1
        x4 = self.up4(fine_features[0], diff0_features[0], coarse_features[0], x3)   #修改，删除diff1
...
class FineUpBlock(nn.Module):
    ...
    def forward(self, x_fine0, x_diff0, x_coarse0, x_fine1):   #修改，删除x_diff1
        """
        ...
        x_f0 = self.layer(x_diff0, x_fine0, x_coarse0) + x_fine0   #修改，删除diff1
