...
class PatchMerging2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.patch_merge(x).view(b, -1, new_h, new_w).permute(0, 2, 3, 1)
        x = self.linear(x)  #线性变换，修改输出特征通道
        return x.permute(0, 3, 1, 2)  # [N, H//downscaling_factor, W//downscaling_factor, out_channels]
...
class SwinModule(nn.Module):
    def __init__(self, in_channels, hidden_dimension, layers, downscaling_factor, num_heads, head_dim, window_size,
        ...
        r"""
        Args:
            in_channels(int): 输入通道数                                                          1
            hidden_dimension(int): 隐藏层维数，patch_partition提取patch时有个Linear学习的维数       64
            layers(int): swin block数，必须为2的倍数，连续的，regular block和shift block            2
            downscaling_factor: H,W上下采样倍数                                                    2 
            num_heads: multi-attn 的 attn 头的个数                                                 4 
            head_dim:   每个attn 头的维数                                                          16
            window_size:    窗口大小，窗口内进行attn运算                                            4
        """
        ...
        self.patch_partition = PatchMerging(in_channels=in_channels, out_channels=hidden_dimension,
                                            downscaling_factor=downscaling_factor)  #1 64 2
        
        self.layers = nn.ModuleList([])
        '一个regular block后接一个shifted block'
    ...
    def forward(self, x, y=None):
        if y is None:
            ...
            #到这里完成了模型中的PATCH EMBEDDING部分，PAN图像被切分成2H*2W*C的输入，MS图像被切分成H*W*C的输入
            #接下来进入SAB模块(self attention block)
