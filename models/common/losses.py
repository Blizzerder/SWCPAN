...
def get_loss_module(full_cfg, logger):  #获取从loss名字到loss实例的字典
    ...
    """
    loss_cfg = full_cfg.get('loss_cfg', {'rec_loss': dict(type='l2', w=1.)})    #获取loss_cfg中的配置信息
    loss_module = dict()

    for loss_name in loss_cfg:      #遍历所有loss
        cfg = loss_cfg[loss_name]   #获取loss名
